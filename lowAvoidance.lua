...
local function GetTalentedCritReduction()
	local _, class = UnitClass("player");
	local uncrit = 0;

	-- SoTF
	if ( class == "DRUID" ) then
		uncrit = select(5, GetTalentInfo(2, 16));
	-- SoH
	elseif ( class == "ROGUE" ) then
		uncrit = select(5, GetTalentInfo(3, 3));
	end
...
local function UpdateHitTable()
	...
	if not mainhand then result.parry = 0; else
		local mainhandtype = select(9, GetItemInfo(mainhand));
	...
	if not offhand then result.block = 0; else
		local offhandtype = select(9, GetItemInfo(offhand));
...
function f:PLAYER_LOGIN()
	...
	self.talentedcritreduct = GetTalentedCritReduction();
end
